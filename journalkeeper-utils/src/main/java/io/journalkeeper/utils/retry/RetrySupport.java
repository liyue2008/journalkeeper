 ...
 * Date: 2019-09-17
 */
public class RetrySupport<D/* 对端地址类型 */> {
    private static final Logger logger = LoggerFactory.getLogger(RetrySupport.class);
    ...
    private final ScheduledExecutorService retryScheduler = Executors.newScheduledThreadPool(1, new NamedThreadFactory("RetryExecutor", true));

    public RetrySupport(RetryPolicy retryPolicy, DestinationSelector<D> destinationSelector) {
    ...
    public final <R /* Response */> CompletableFuture<R> retry(RpcInvoke<R, D> invoke, CheckRetry<? super R> checkRetry) {
        return retry(invoke, checkRetry, null);
    }

    public final <R /* Response */> CompletableFuture<R> retry(RpcInvoke<R, D> invoke, CheckRetry<? super R> checkRetry, D fixDestination) {

        RpcInvokeWithRetryInfo<R, D> retryInvoke = invoke instanceof RetrySupport.RpcInvokeWithRetryInfo ? (RpcInvokeWithRetryInfo<R, D>) invoke : new RpcInvokeWithRetryInfo<>(invoke);

        D dest = fixDestination == null ? getDestination(retryInvoke) : fixDestination;

        return retryInvoke.invoke(dest)
                ...
                .thenCompose(r -> {
                    ...
                    if (retry) {
                        ...
                        if (delay > 0) {
                            return Async.scheduleAsync(retryScheduler, () -> retry(retryInvoke, checkRetry, fixDestination), delay, TimeUnit.MILLISECONDS);
                        } else if (delay == 0) {
                            return retry(retryInvoke, checkRetry, fixDestination);
                        }
                    }
